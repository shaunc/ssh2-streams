...
ReadStream.prototype._read = function(n) {
  ...
  // the actual read.
  var self = this;
  const isReady = this.sftp.readData(
    this.handle, pool, pool.used, toRead, this.pos, onread);
...
function WriteStream(sftp, path, options) {
  ...
  // transform "continue" into "drain"
  this.sftp.on('continue', () => {
    this.emit('drain')
  })
}
...
WriteStream.prototype._write = function(data, encoding, cb) {
  ...
  if (!Buffer.isBuffer(this.handle)) {
    this.once('open', function() {
      if(this._write(data, encoding, cb) !== false) {
        this.emit('drain')
      }
    });
    return false
  ...
  var self = this;
  const isReady = this.sftp.writeData(this.handle,
                      data,
                      0,
                      data.length,
                      this.pos,
...
// Don't provide until backpressure fixed.
// WriteStream.prototype._writev = function(data, cb) {
//   if (!Buffer.isBuffer(this.handle)) {
//     this.once('open', function() {
//       if(this._writev(data, cb) !== false) {
//         this.emit('drain')
//       }
//     });
//     return false
